...
async def create_request_get(request: Request, current_user: models.User = Depends(get_current_user)):
    return templates.TemplateResponse(
        ...
        context={
            "created": False,
            "csrf_token": get_csrf_token(request),
            "pro_amount": plan_price("PRO"),
            "enterprise_amount": plan_price("ENTERPRISE"),
        },
    )


def plan_price(plan: str) -> int:
    """Server-side source of truth for plan pricing. Never trust a client-
    submitted amount — a request could otherwise select ENTERPRISE and post
    any amount it likes."""
    prices = {
        "PRO": int(os.getenv("PRO_AMOUNT_NGN", "2500")),
        "ENTERPRISE": int(os.getenv("ENTERPRISE_AMOUNT_NGN", "7500")),
    }
    return prices.get(plan, prices["PRO"])


@router.post("/create-request", response_class=HTMLResponse)
async def create_request_post(request: Request, plan: str = Form(...), csrf_token: str = Form(...), db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
    require_csrf(request, csrf_token)

    plan = (plan or "").upper().strip()
    if plan not in {"PRO", "ENTERPRISE"}:
        plan = "PRO"
    amount = plan_price(plan)  # computed server-side — the form no longer accepts a client amount
