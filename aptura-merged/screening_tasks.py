...
def process_screening(screening_id: int, files: list[dict]) -> dict:
    """
    files: [{"candidate_id": int, "filename": str}, ...]
    The actual file bytes live on Candidate.file_content_b64 — they're not
    passed through Celery directly since raw bytes aren't JSON-serializable
    and the celery_app is configured with task_serializer="json".
    """
    db = SessionLocal()
    processed = 0
    errors = 0
    ...
    try:
        ...
        for item in files:
            ...
            try:
                ...
                candidate.error_message = None
                # Clear the stashed file content once processed — no need to
                # keep a base64 copy of every resume sitting in the DB forever.
                candidate.file_content_b64 = None
                processed += 1
