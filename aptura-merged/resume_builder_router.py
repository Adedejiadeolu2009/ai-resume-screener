...
def _chat_json_gemini(prompt: str) -> dict:
    """JSON chat completion via Gemini — this is what actually runs for you,
    since GEMINI_API_KEY is the only provider configured in your .env."""
...
def _chat_json(prompt: str) -> dict:
    """
    Tries providers in this order: Groq (if configured) -> OpenAI/DeepSeek
    (if configured) -> Gemini (if configured). Your real .env only has
    GEMINI_API_KEY, so this falls through to Gemini — the Groq/OpenAI
    branches are dead code for you right now, kept only so this still
    works unmodified if you add those keys later.
    """
    ...
    if gemini_key:
        return _chat_json_gemini(prompt)

    raise RuntimeError("No AI provider configured. Set GEMINI_API_KEY (or GROQ_API_KEY/GROQ_API_BASE, or OPENAI_API_KEY) in .env.")
