...
logger = logging.getLogger("aptura.admin")
...
def is_admin(user: models.User) -> bool:
    """
    A user is admin if the DB flag is set, or (as a bootstrap path only)
    their email matches ADMIN_EMAIL. `sync_admin_flag()` promotes the
    ADMIN_EMAIL account's DB flag on every app startup, so in practice
    the DB flag is the durable source of truth and ADMIN_EMAIL is just
    how you seed the very first admin. Additional admins can be granted
    later by setting is_admin=True directly in the database — no env
    var or code change required.
    """
...
def sync_admin_flag(db: Session) -> None:
    """Call once at startup. Promotes the ADMIN_EMAIL user's is_admin
    DB flag so admin access no longer depends on the env var being
    present at request time."""
    admin_email = (os.getenv("ADMIN_EMAIL", "") or "").strip().strip('"').strip("'").lower()
    if not admin_email:
        return
    user = db.query(models.User).filter(func.lower(models.User.email) == admin_email).first()
    if user and not user.is_admin:
        user.is_admin = True
        db.commit()
        logger.info("Promoted %s to admin via ADMIN_EMAIL bootstrap", admin_email)
...
async def admin_requests(
    ...
    return templates.TemplateResponse(
        ...
        context={"requests": payload, "csrf_token": _main.get_csrf_token(request)},
    )
...
async def confirm_upgrade(
    ...
    try:
        ...
        logger.exception("Failed to write payment audit record for %s", request_ref)
    ...
    try:
        if payment and payment.user and payment.user.email:
            ...
            try:
                ...
                logger.exception("Failed to render confirmation email template for %s", request_ref)
                html = None
        ...
        logger.exception("Confirmation email flow failed for %s", request_ref)
...
async def record_offline_payment(
    ...
    try:
        ...
        logger.exception("Failed to send offline-payment confirmation email for %s", reference)
...
async def payments_list(
    ...
    return templates.TemplateResponse(
        request=request,
        name="admin_payments.html",
        context={"payments": payload, "csrf_token": _main.get_csrf_token(request)},
    )
...
async def resend_confirmation(
    ...
    try:
        ...
        logger.exception("Failed to render resend-confirmation email template for %s", request_ref)
        html = None
    ...
    try:
        ...
        logger.exception("Failed to send resend-confirmation email for %s", request_ref)
        email_status = "failed"
    ...
    try:
        ...
        logger.exception("Failed to write resend-confirmation audit record for %s", request_ref)
